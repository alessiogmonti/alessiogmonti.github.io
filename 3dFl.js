...
function main() {
  ...
  const keyLight = new THREE.DirectionalLight(new THREE.Color('#1A535c'), 1);
  keyLight.position.set(-100, 0, 100);

  const fillLight = new THREE.DirectionalLight(new THREE.Color('#ff6b6b'), 0.75);
  fillLight.position.set(100, 0, 100);

  const backLight = new THREE.DirectionalLight(new THREE.Color('#ffe66d'), 1);
...
//don't think draco models improve performance by much.
    gltfLoader.load('FLIGHTDRACO.gltf', (gltf) => {
      root = gltf.scene;
